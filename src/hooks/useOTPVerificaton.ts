...
const useOTPVerificaton = () => {
    const [otpVerificationLoader, setOtpVerificationLoader] = useState<boolean>(false);
    ...
    const verifyOtp = async (mobileNumber: string, otpNumbers: string) => {
        ...
        try {
            ...
            const verifyResponse = await bharatStay.post(apiEndpoints.POST.otpVerificfation, payload);
